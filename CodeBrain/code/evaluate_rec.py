...
from utils.utils import rand_seed, show_img, check_is_legal, masking_img
...
def validate_rec(Trainer, val_loader, opt, writer, current_iter):
    """
    Validate function for reconstruction task.
    """
            ...
            for pattern in missing_patterns:
                ...
                for ch in range(c_modal):
                    if pattern[ch] == 0:  # Only evaluate missing modalities
                        ...
                        output_slice = out_volumes[:, ch, :, :].unsqueeze(1)
...
def evaluate_rec(rec_model, test_loader, opt, result_path, device):
    """
    ...
    with torch.no_grad():
        for test_step, sample in enumerate(tqdm(test_loader)):
            ...
            for pattern in missing_patterns:
                ...
                # Create partial volume by zeroing out missing channels
                masked_volumes = masking_img(volumes, pattern)
                out_volumes, _ = rec_model(masked_volumes, volumes)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, help='save path', default='../models/cb_rec_IXI_20260318-112206')
    ...
    ckpt = torch.load(os.path.join(args.save_path, '{}_{}_best.pth'.format(args.rec_name, opt.DATASET)), weights_only=True, map_location=device)
    ...
    evaluate_rec(rec_model, test_loader, opt, result_path, device)